...
double sigma_lv     				= Y[87];///1.5; //data->p_ursino[113]; // % units: mmHg/ml
double sigma_rv     				= Y[88];///0.935; //data->p_ursino[114]; //  units: mmHg/ml
...
data->Qupi	= (Y[12] - Y[3])	/(data->p_R[13]* sigma_R)	;
...
Y[7] is P_ab
*/
data->Qsp1 = (Y[12] - Y[5])	/(data->p_R[15] * sigma_R)	;
...
data->Qk1 = (Y[12] - Y[4])/data->p_R[17];
data->Qk2 = (Y[4] - Y[7])/data->p_R[18];
...
Y[6] is P_ll,
Y[7] is P_ab.  */
data->Qll1	 	= (Y[12] - Y[6])	/(data->p_R[43]* sigma_R);
...
dY[3]     = (data->Qupi - data->Qupo) / data->p_C[10];


/*************** kidney ************************************/
/*************** original formula **************************/
dY[4]     = (data->Qk1 - data->Qk2) / data->p_C[13]; // % Y[5-1] is Pk, Ck is non-zero. eq. 23, appendix 2, p585.
...
dY[5]     = (data->Qsp1 - data->Qsp2) / data->p_C[11];

// % Y[6] is Pll, legs pressure.
dY[6]     = (data->Qll1 - data->Qll2) / data->p_C[26];

// % Y[7] is Pab.
dY[7]     = (data->Qk2 + data->Qsp2 + data->Qll2 - data->Qab) / data->p_C[2] ;
...
dY[12]    = (1.0/data->p_C[6]) * (data->Qlo - (data->Qupi + data->Qsp1 + data->Qk1 + data->Qll1 + Q_Ceri)) + 0.333*dY[8];

// % Y[13] is Psup, eq. 27 appendix 2.
dY[13]    = ((data->Qupo + Q_Cero) - data->Qsup) / data->p_C[1] + dY[8];
// % Y[14] is Pinf, eq. 28 appendix 2.
dY[14]    = (data->Qab - data->Qinf) / data->p_C[3] + dY[8];
